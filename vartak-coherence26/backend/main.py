...
app = FastAPI(title="Indian Budget Intelligence API", lifespan=lifespan)
    ...
    async def generate_alerts(self):
        ...
        try:
            while True:
                ...
                except (sqlite3.Error, pd.errors.DatabaseError, KeyError, ValueError):
                    pass
        finally:
            self._running = False
...
async def anomalies_list_dashboard(limit: int = 20):
...
def _budget_system_prompt() -> str:
    try:
        ...
        records = len(df)
    except (sqlite3.Error, pd.errors.DatabaseError, KeyError, ValueError, TypeError):
...
async def anomaly_detection_api(
    ...
    iso_model = IsolationForest(contamination=sensitivity, random_state=42, n_estimators=100)
...
async def smart_reallocation(
    ...
    if len(dept_df) > 1:
        ...
        _median_remaining = float(dept_df['total_remaining'].median())
    else:
        median_util = 77.0
        _median_remaining = 0
